...
class TerminalStore extends EventEmitter {
	...
	constructor() {
			...
			this.addOutput([
				{
					...
					type: "labelValue",
					label: "Provider",
					value: this.currentModel.provider,
					color: "text-cyan-400",
				},
				{
					text: "Model:",
		...
		this.chatManager.on("error", (error: Error | unknown) => {
			this.addOutput([
				{
					text: "ERROR: Failed to initialize chat system",
					type: "error",
					color: "text-red-400",
				},
				{
	...
	async handleChat(message: string): Promise<(string | ColoredLine)[]> {
		return [
			...
			{ text: `Processing: ${message}`, type: "text", color: "text-[#F6BD7A]" },
			{ text: "", type: "text", color: "text-[#F6BD7A]" },
			{
				text: "Response coming soon...",
				type: "text",
				color: "text-[#F6BD7A]",
			},
			{ text: "", type: "text", color: "text-[#F6BD7A]" },
