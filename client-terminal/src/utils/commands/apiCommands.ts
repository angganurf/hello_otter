...
export const apiCommands: Command[] = [
	{
		...
		action: (args) => {
			if (args[0] === "-h") {
				return [
					"Usage: otter api [-h] [generate|show|renew]",
					"",
					...
					"",
					"Example:",
					"  otter api generate    Generate new key",
					"  otter api renew       Replace existing key",
				];
			}
			...
			switch (subcommand) {
				case "generate": {
					...
					if (existingKey) {
						return [
							...
							'Use "otter api show" to view it or',
							'"otter api renew" to generate a new one.',
						];
					}
					...
					return [
						...
						`WebSocket: ${config.ai.providers.galadriel.baseUrl.replace(
							"http",
							"ws"
						)}/ws`,
						"",
				...
				case "info": {
					...
					return [
						...
						"Current API Key: " +
							(userKey?.key ||
								'No API key found. Use "otter api generate" to create one.'),
						"",
						"API Endpoints:",
						`REST API: ${config.ai.providers.galadriel.baseUrl}`,
						`WebSocket: ${config.ai.providers.galadriel.baseUrl.replace(
				...
				case "show": {
					...
					if (!key) {
						return [
							"No API key found.",
							'Use "otter api generate" to create one.',
						];
					}
				...
				default:
					return [
						...
						"Available commands:",
						"  otter api generate    Generate new key",
						"  otter api info        Show API details",
						"  otter api show        Show current key",
						"  otter api renew       Replace existing key",
						"",
						'Type "otter api -h" for detailed help',
					];
			}
		},
	},
