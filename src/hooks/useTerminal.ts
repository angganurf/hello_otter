...
export const useTerminal = (onNavigate?: (section: string) => void) => {
	...
	const [output, setOutput] = useState<(string | ColoredLine)[]>(
		terminalStore.getOutput()
	);
	...
	const handleCommand = useCallback(
		async (cmd: string) => {
			...
			// Check if this is a otter command
			if (trimmedCmd.toLowerCase().startsWith("otter ")) {
				const [, command, ...args] = trimmedCmd.split(" ");

				// Handle navigation commands
				if (
					["home", "characters", "otter", "logs"].includes(command) &&
					onNavigate
				) {
					onNavigate(command);
					return;
				...
				if (commandHandler) {
					...
					if (command === "view" && args[0]) {
						...
						if (character && isMobile && onNavigate) {
							terminalStore.addOutput([
								"Note: View command is optimized for desktop experience",
							]);
					...
					terminalStore.addOutput([
						'Command not recognized. Type "otter help" for available commands.',
					]);
				}
			} else {
				...
				try {
					...
					const response = await galadrielAPI.chat(
						activeCharacter.id,
						trimmedCmd
					);
