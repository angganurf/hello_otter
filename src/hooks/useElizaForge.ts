...
export const useElizaForge = () => {
	...
	const handleCommand = useCallback(
		async (cmd: string) => {
			...
			if (state.step === 0) {
				addOutput(['Type "start" to begin or "help" for commands.', ""]);
				setInput("");
				return;
			}
...
- Keep responses focused and in character`;
				...
				character.messageExamples.forEach((exchange, i) => {
					if (!Array.isArray(exchange) || exchange.length !== 2) {
						throw new Error(
							`Message example ${
								i + 1
							} must be an array with exactly 2 messages`
						);
					}
					...
					if (!userMsg?.user || userMsg.user !== "{{user1}}") {
						throw new Error(
							`First message user must be {{user1}} in example ${i + 1}`
						);
					}

					if (!charMsg?.user || charMsg.user !== data.name) {
						throw new Error(
							`Second message user must be character name in example ${i + 1}`
						);
					}
			...
			addOutput([
				...
				"",
				`Forge Rate Limit: ${forgeLimiter.getRemainingRequests()}/5 attempts remaining`,
				`Window resets in ${Math.ceil(
					forgeLimiter.getTimeUntilReset() / 60000
				)} minutes`,
				"",
			...
			addOutput([
				...
				error instanceof Error
					? error.message
					: "Failed to generate character data",
