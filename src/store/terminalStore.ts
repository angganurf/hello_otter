...
class TerminalStore extends EventEmitter {
	...
	private initializeBaseOutput(): void {
		const activeCharacter = characters.find(
			(c) => c.id === userService.getActiveCharacter()
		);
		...
		this.addOutput([
			{
				text: "Hello Otter Terminal v1.0.0",
				color: "text-cyan-400",
				type: "header",
			},
			{ text: "", color: "text-[#f6bd7a]", type: "text" },
			{
			...
			{
				text: "Chat directly with Otter by typing without the otter prefix",
				color: "text-[#f6bd7a]",
				type: "text",
			},
			{ text: "", color: "text-[#f6bd7a]", type: "text" },
			...
			{
				text: `Agent: ${activeCharacter?.name || defaultCharacter.name}`,
				color: "text-cyan-400",
				type: "label",
			},
			{
				text: `Provider: ${
					defaultCharacter.modelProvider?.toUpperCase() || "GALADRIEL"
				}`,
				color: "text-cyan-400",
				type: "label",
			},
			{
				text: `Model: ${defaultCharacter.model || "llama3.1:70b"}`,
				color: "text-cyan-400",
				type: "label",
			},
			{ text: "", color: "text-[#f6bd7a]", type: "text" },
	...
	async handleChat(message: string): Promise<(string | ColoredLine)[]> {
		...
		if (!character) {
			return [
				{
					text: "ERROR: No active character selected",
					type: "error",
					color: "text-red-400",
				},
				{
					text: 'Use "otter chat <agent>" to select a character',
					type: "text",
					color: "text-[#f6bd7a]",
				},
			];
		...
		if (!aiCharacter) {
			return [
				{
					text: "ERROR: Character AI profile not found",
					type: "error",
					color: "text-red-400",
				},
				{
		...
		try {
			...
			return [
				...
				},
				{
					text: "--------------------------------",
					type: "separator",
					color: "text-[#f6bd7a]/30",
				},
				{
					text: `[${character.name.toUpperCase()}] Processing input: "${message}"`,
					type: "text",
					color: "text-cyan-400",
				},
				{ text: "", type: "text", color: "text-[#f6bd7a]" },
				{ text: response.message, type: "text", color: "text-[#f6bd7a]" },
				{ text: "", type: "text", color: "text-[#f6bd7a]" },
			...
			return [
				{
					text: "ERROR: Failed to process chat",
					type: "error",
					color: "text-red-400",
				},
				{
	...
	updateActiveAgent(characterId: string): void {
		...
		if (character) {
			this.addOutput([
				...
				{
					text: `Now chatting with: ${character.name}`,
					type: "text",
					color: "text-[#f6bd7a]",
				},
				{ text: "", type: "text", color: "text-[#f6bd7a]" },
	...
	private handleModelUpdate(modelInfo: {
		provider: string;
		model: string;
	}): void {
