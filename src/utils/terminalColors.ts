...
export const getLineColor = (
	line: ColoredLine | string | null | undefined
): string => {
	if (!line) {
		return "text-[#F6BD7A]";
	}
	...
	if (content.match(/^-+$/)) return "text-[#F6BD7A]/30";
	...
	if (content.includes("SUCCESS:")) return "text-[#F6BD7A]";
	if (content.includes("ACCESSING") || content.includes("Loading"))
