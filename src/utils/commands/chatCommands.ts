		...
		action: (args) => {
			...
			if (!agentId) {
				...
				return [
					...
					"",
					'Type "otter chat -h" to see available agents',
					"Chat directly by typing without the otter prefix",
				];
			}
			...
			if (!agent) {
				return [
					...
					'Type "otter chat -h" for available agents',
				];
			}
			...
			return [
				...
				"",
				"Start chatting by typing without the otter prefix",
			];
		},
	},
];
