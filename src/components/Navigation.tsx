...
export const Navigation: React.FC<NavProps> = ({
	...
	const navItems = [
		...
		{ id: "image2video", icon: Bot, label: "Image2Video" },
	...
	return (
		<nav className="fixed top-0 w-full bg-black/50 backdrop-blur-sm border-b border-[#F6BD7A]/30 px-2 xs:px-3 sm:px-4 py-1 xs:py-1.5 sm:py-2 z-50">
			<div className="max-w-5xl mx-auto flex flex-col items-center space-y-2">
				<div className="hidden sm:flex justify-center mb-2">
					<h1 className="text-base xs:text-lg sm:text-xl font-mono text-center">
						<span className="text-glow text-cyan-400">HELLO OTTER v1.0.0</span>
					</h1>
				</div>
				<div className="flex items-center justify-between w-full sm:justify-center">
					<h1 className="text-xl font-mono flex items-center gap-2 sm:hidden">
						<span className="text-glow text-cyan-400 text-xs xs:text-sm">
							HELLO OTTER v1.0.0
						</span>
					</h1>
					{isMobile ? (
						<button
							...
							className="text-[#F6BD7A] hover:text-cyan-400 transition-colors p-1 xs:p-1.5"
                    ...
                    ${
											activeSection === id
												...
												: "text-[#F6BD7A] hover:text-cyan-400"
										}`}
									aria-label={label}
									role="menuitem"
								>
				...
				{isMobile && isMenuOpen && (
					<div
						className="absolute left-0 right-0 top-full bg-black/95 border-b border-[#F6BD7A]/30 py-2 animate-slide-in"
						role="menu"
					>
						{navItems.map(({ id, icon: Icon, label }) => (
							<button
