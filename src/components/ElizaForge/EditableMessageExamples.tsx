...
export const EditableMessageExamples: React.FC<
	EditableMessageExamplesProps
> = ({ examples, onChange, characterName = "Character" }) => {
	const handleAdd = () => {
		const newExample = [
			{
				user: "{{user1}}",
					...
					<div key={index} className="space-y-2 bg-black/20 p-2 rounded">
						<div className="flex flex-col xs:flex-row xs:items-center justify-between gap-1">
							...
							<button
								...
								className="text-[#F6BD7A] hover:text-red-400 transition-colors"
							>
								<X className="w-4 h-4" />
							</button>
						</div>
						<div className="space-y-2">
							...
							<div className="flex flex-col xs:flex-row xs:items-start gap-1 xs:gap-2">
								<span className="text-[#F6BD7A] text-xs xs:w-16 shrink-0">
									User:
								</span>
							...
							<div className="flex flex-col xs:flex-row xs:items-start gap-1 xs:gap-2 mt-2">
								<span className="text-cyan-400 text-xs xs:w-16 shrink-0 truncate">
									{characterName}:
								</span>
								<div className="flex-1">
									<EditableField
