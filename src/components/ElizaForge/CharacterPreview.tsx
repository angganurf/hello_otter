...
export const CharacterPreview: React.FC<CharacterPreviewProps> = ({
	...
	const { isDragActive, dragRef, openFileDialog } = useDropzone(
		(files) => {
			if (files?.[0]) {
			...
			<div
				...
				className={`flex items-center justify-center h-full border-2 border-dashed transition-colors cursor-pointer ${
					isDragActive
						...
						: "border-[#F6BD7A]/30 hover:border-cyan-400/30"
				}`}
			>
				<div className="text-center">
					<p className="text-[#F6BD7A]/50 font-mono text-[10px] xs:text-xs sm:text-sm text-center px-2 max-w-[150px] xs:max-w-none">
	...
	return (
		...
		>
			...
			<div className="space-y-1 xs:space-y-2">
				...
				<div className="bg-black/40 p-2 xs:p-3 rounded-lg border border-[#F6BD7A]/30 space-y-1">
					<div className="flex flex-col xs:flex-row xs:items-center gap-1 xs:gap-2">
						<span className="text-cyan-400 text-xs xs:w-16 shrink-0">
							Name:
						</span>
						<EditableField
					...
					{data.username && (
						<p className="text-[#F6BD7A] text-xs">
			...
			<div className="space-y-1.5 xs:space-y-2">
				...
				<div className="bg-black/40 p-2 xs:p-3 rounded-lg border border-[#F6BD7A]/30 space-y-2">
					<EditableArray
			...
			<div className="space-y-1.5 xs:space-y-2">
				...
				<div className="bg-black/40 p-2 xs:p-3 rounded-lg border border-[#F6BD7A]/30 overflow-x-hidden">
			...
			<div className="space-y-1.5 xs:space-y-2">
				...
				<div className="bg-black/40 p-2 xs:p-3 rounded-lg border border-[#F6BD7A]/30 space-y-3 xs:space-y-4">
					<EditableArray
						values={data.style?.all || []}
						onChange={(values) =>
							handleChange("style", { ...data.style, all: values })
						}
						label="All"
					/>
					<EditableArray
						...
						onChange={(values) =>
							handleChange("style", { ...data.style, chat: values })
						}
						label="Chat"
					/>
					<EditableArray
						...
						onChange={(values) =>
							handleChange("style", { ...data.style, post: values })
						}
						label="Post"
					/>
				</div>
			...
			<div className="space-y-2">
				...
				<div className="bg-black/40 p-3 rounded-lg border border-[#F6BD7A]/30 space-y-4">
					<EditableArray
						values={data.topics || []}
