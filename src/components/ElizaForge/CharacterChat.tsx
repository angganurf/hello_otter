...
export const CharacterChat: React.FC<CharacterChatProps> = ({ character }) => {
	...
	const generateResponse = useCallback(
		async (userMessage: string) => {
			...
			const styles = getRandomItems(
				[...character.style.all, ...character.style.chat],
				3
			);
			...
			const bioItems = Array.isArray(character.bio)
				? getRandomItems(character.bio, 2)
				: [character.bio];

			// Build context template
			const systemPrompt = `You are ${
				character.name
			}. Respond in character using the following traits and style:
...
IMPORTANT: Always stay in character and respond as ${
				character.name
			}. Use the style elements and reference background/lore naturally.`;
	...
	if (!character) {
		return (
			<div className="w-full bg-black/80 backdrop-blur-md rounded-lg border border-[#F6BD7A]/30 p-4 text-center">
				<p className="text-[#F6BD7A]/50 font-mono text-sm">
	...
	return (
		<div className="w-full flex flex-col bg-black/80 backdrop-blur-md rounded-lg border border-[#F6BD7A]/30 shadow-lg shadow-[#F6BD7A]/20">
			{/* Header */}
			...
			<form
				...
				className="flex items-center gap-2 p-3 border-t border-[#F6BD7A]/30 bg-black/40"
			>
				<input
					ref={inputRef}
					type="text"
					...
					className="flex-1 bg-black/40 text-[#F6BD7A] placeholder-[#F6BD7A]/50 px-3 py-2 rounded-lg
                   border border-[#F6BD7A]/30 outline-none font-mono text-sm
