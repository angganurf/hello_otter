...
export const Logs: React.FC<LogsProps> = ({ onNavigate }) => {
	...
	const downloadCharacter = useCallback(() => {
		...
		a.href = url;
		a.download = `${selectedCharacter.name
			.toLowerCase()
			.replace(/\s+/g, "_")}.character.json`;
	...
	return (
		<div className="w-full">
			<div className="flex gap-4">
				<div
					className={`flex flex-col bg-black/80 backdrop-blur-md rounded-lg border border-[#F6BD7A]/30 shadow-lg shadow-[#F6BD7A]/20 h-[600px] ${
						selectedCharacter && !isMobile ? "w-1/2" : "w-full"
					}`}
				>
					...
					<div className="flex-1 overflow-y-auto p-4">
						<div className="space-y-4">
								...
								{characterLogs.map((log) => {
									...
									return (
										<button
											...
											className="w-full bg-black/40 rounded-lg border border-[#F6BD7A]/30 p-3 text-left hover:border-cyan-400/50 transition-colors"
										>
											<div className="flex items-center justify-between">
												<span className="text-cyan-400 font-mono text-sm">
													{character.name}
												</span>
												<span className="text-[#F6BD7A]/60 font-mono text-xs">
													{new Date(log.timestamp).toLocaleString()}
												</span>
											</div>
											<p className="text-[#F6BD7A]/80 font-mono text-xs mt-1 line-clamp-2">
							...
							{conversations.map((conversation) => {
								...
								const isExpanded = expandedConversations.includes(
									firstMessage.conversation_id
								);

								return (
									<div
										key={firstMessage.conversation_id}
										className="bg-black/40 rounded-lg border border-[#F6BD7A]/30"
									>
										<button
											onClick={() =>
												toggleConversation(firstMessage.conversation_id)
											}
											className="w-full flex items-start gap-4 p-4"
										>
											{isExpanded ? (
												<ChevronUp className="w-4 h-4 text-[#F6BD7A] mt-1" />
											) : (
												<ChevronDown className="w-4 h-4 text-[#F6BD7A] mt-1" />
											)}
											...
											<div className="flex-1 text-left">
												<div className="flex flex-col gap-1">
													<div className="flex items-center gap-2">
														<span className="text-cyan-400 font-mono text-sm">
															{shortenUserId(firstMessage.user_id)} →{" "}
															{getCharacterName(firstMessage.character_id)}
														</span>
														<span className="text-[#F6BD7A]/60 font-mono text-xs">
															{formatTimestamp(lastMessage.timestamp)}
														</span>
													</div>
													<div className="text-[#F6BD7A]/80 font-mono text-xs truncate">
										...
										{isExpanded && (
											<div className="px-12 pb-4">
												<div className="space-y-3 pt-2 border-t border-[#F6BD7A]/30">
													{conversation.map((message) => (
														<div key={message.id} className="space-y-1">
															<div className="flex items-center gap-2">
																<span className="text-cyan-400 font-mono text-xs">
																	{message.role === "user"
																		? shortenUserId(message.user_id)
																		: getCharacterName(message.character_id)}
																</span>
																<span className="text-[#F6BD7A]/60 font-mono text-xs">
																	{formatTimestamp(message.timestamp)}
																</span>
															</div>
															<p className="text-[#F6BD7A]/80 font-mono text-sm">
																{message.message}
															</p>
														</div>
													))}
				...
				{selectedCharacter && !isMobile && (
					<div className="w-1/2 flex flex-col bg-black/80 backdrop-blur-md rounded-lg border border-[#F6BD7A]/30 shadow-lg shadow-[#F6BD7A]/20 h-[600px]">
						<div className="flex items-center gap-2 p-3 border-b border-[#F6BD7A]/30 bg-black/40">
							<Bot className="w-4 h-4 text-[#F6BD7A]" />
							<span className="text-[#F6BD7A] font-mono text-base">
								Character Preview
							</span>
							<div className="ml-auto flex items-center gap-2">
								{selectedCharacter && (
									<button
										...
										className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-black/40 border border-[#F6BD7A]/30 
                           hover:border-cyan-400/50 transition-colors group"
									>
										...
										<span className="hidden xs:inline text-[#F6BD7A] font-mono text-[10px] xs:text-xs group-hover:text-pink-400">
											Download
										</span>
									</button>
								)}
