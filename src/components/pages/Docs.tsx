						...
						<div className="space-y-2 pl-4">
							{section.items.map((item, j) => (
								<p key={j} className="flex items-start gap-2">
									<span className="text-cyan-400 w-4">{item.number}.</span>
						...
						<div className="space-y-2">
							{section.commands.map((cmd, j) => (
								<p key={j} className="flex items-start gap-2">
									...
									<span className="text-pink-400">- {cmd.description}</span>
								</p>
							))}
						</div>
					)}
		...
		<div className="w-full">
			<div className="flex flex-col bg-black/80 backdrop-blur-md rounded-lg border border-[#F6BD7A]/30 shadow-lg shadow-[#F6BD7A]/20 h-[600px]">
				...
				<div className="flex-1 flex overflow-hidden relative">
					...
					<div
						className={`${
							isMobile
								...
								: "w-48 border-r border-[#F6BD7A]/30"
						} overflow-y-auto`}
					>
						<div className="p-2">
							{isMobile && showNav && (
								...
								<button
									...
									className={`w-full text-left px-3 py-2 rounded font-mono text-sm transition-colors ${
										selectedCategory.id === category.id
											? "bg-[#F6BD7A]/20 text-cyan-400"
											: "text-[#F6BD7A] hover:bg-[#F6BD7A]/10"
									}`}
								>
									{category.title}
								</button>
					...
					{/* Sections */}
					{!isMobile && (
						<div className="w-48 border-r border-[#F6BD7A]/30 overflow-y-auto">
							<div className="p-2">
