				...
				<div className="flex-1 p-2 xs:p-3 sm:p-4 overflow-y-auto">
					<div className="max-w-3xl mx-auto space-y-3">
						<div className="text-center">
							<h2 className="text-sm xs:text-base font-mono text-cyan-400 mb-1">
								$OTTER Token
							</h2>
						</div>
						<div className="grid grid-cols-2 gap-1.5">
							<div className="bg-black/40 p-1.5 rounded-lg border border-[#F6BD7A]/30">
								<h3 className="text-[#F6BD7A] font-mono text-[10px] xs:text-xs mb-1">
									Supply
								</h3>
								<p className="text-cyan-400 font-mono text-[10px] xs:text-xs whitespace-nowrap">
									1,000,000,000
								</p>
							</div>
							<div className="bg-black/40 p-1.5 rounded-lg border border-[#F6BD7A]/30">
						...
						<div className="bg-black/40 p-1 rounded-lg border border-[#F6BD7A]/30">
							<div className="grid grid-cols-2 gap-4 font-mono text-[10px] xs:text-xs">
								<div className="space-y-1.5">
									<div className="flex items-center justify-between">
										<span className="text-[#F6BD7A]">Chain:</span>
										<span className="text-cyan-400">Solana</span>
									</div>
									<div className="flex items-center justify-between">
										<span className="text-[#F6BD7A]">Network:</span>
										<span className="text-cyan-400">Mainnet</span>
									</div>
								</div>
								<div>
									<h3 className="text-[#F6BD7A] mb-0.5 xs:mb-1">Description</h3>
									<p className="text-cyan-400/80 leading-relaxed">
										$OTTER Powers the Hello Otter ecosystem.
									</p>
								</div>
							</div>
							<div className="font-mono text-[10px] xs:text-xs">
								<span className="text-[#F6BD7A] block mb-1">Contract</span>
								<button
									onClick={() => copyToClipboard(contractAddress)}
									className="flex items-center justify-between gap-1 px-1 py-0.5 bg-black/40 rounded border border-[#F6BD7A]/30 hover:border-cyan-400/50 transition-colors group overflow-hidden"
								>
									<span className="text-cyan-400 font-mono text-xs break-all">
										{contractAddress}
									</span>
									{copied ? (
										<Check className="w-2.5 h-2.5 xs:w-3 xs:h-3 text-[#F6BD7A] flex-shrink-0" />
									) : (
						...
						<div className="bg-black/40 p-1 rounded-lg border border-[#F6BD7A]/30">
							<h3 className="text-cyan-400 font-mono text-[10px] xs:text-xs mb-1">
								Wallets
							</h3>
							<div className="space-y-1 font-mono text-[10px] xs:text-xs">
								{wallets.map((wallet, index) => (
									<div
										key={wallet.name}
										className={`flex flex-col ${index !== 0 ? "mt-1" : ""}`}
									>
										<span className="text-[#F6BD7A] font-semibold">
											{wallet.name}
										</span>
										<a
						...
						<div className="grid grid-cols-1 xs:grid-cols-2 gap-1.5">
							<a
								...
								target="_blank"
								rel="noopener noreferrer"
								className="flex items-center justify-center gap-1.5 px-2 py-1.5 bg-black/40 rounded-lg border border-[#F6BD7A]/30 
                         hover:border-cyan-400/50 transition-colors group"
							>
								...
								<span className="text-[#F6BD7A] font-mono text-[10px] xs:text-xs group-hover:text-pink-400">
									Join Telegram
								</span>
							</a>
							<a
