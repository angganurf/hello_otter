...
export const AgentModal: React.FC<AgentModalProps> = ({
		...
		switch (character.id) {
			...
			case "symbaiex":
				return {
					type: "OTTER",
					icon: Zap,
					bgColor: "bg-gradient-to-r from-cyan-950/80 to-green-450/80",
					textColor:
						"bg-gradient-to-r from-cyan-400 to-[#F6BD7A] text-transparent bg-clip-text",
					borderColor: "border-cyan-400/50 border-r-[#F6BD7A]/50",
				};
			default:
				return {
					type: "AI",
					icon: Brain,
					bgColor: "bg-black/80",
					textColor: "text-[#F6BD7A]",
					borderColor: "border-[#F6BD7A]",
				};
		}
	};
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
			...
			{/* Modal */}
			<div
				className="relative w-full max-w-4xl h-[90vh] bg-black/90 border border-[#F6BD7A]/30 overflow-hidden
                    rounded-lg shadow-lg shadow-[#F6BD7A]/20 animate-slide-in flex flex-col"
			>
				{/* Header */}
				<div className="flex items-center justify-between px-4 py-3 border-b border-[#F6BD7A]/30 bg-black/40">
					...
					<div className="ml-auto flex items-center gap-2">
						<button
							...
							className="px-2 py-1 rounded text-[#F6BD7A] text-[10px] xs:text-xs sm:text-sm
					...
					<button
						onClick={onClose}
						className="text-[#F6BD7A] hover:text-[#F6BD7A] transition-colors"
					>
						<X className="w-6 h-6" />
					</button>
				</div>
				...
				<div className="flex-1 min-h-0">
					<div className="grid grid-cols-1 lg:grid-cols-2 h-full">
						...
						<div className="p-2 lg:p-3 space-y-2 lg:space-y-3 overflow-y-auto">
							<div className="relative rounded-lg border border-[#F6BD7A]/30 overflow-hidden bg-black/40 aspect-[16/9] lg:aspect-[4/3]">
								<img
							...
							<div className="space-y-2 lg:space-y-3">
								<div className="flex items-center justify-between">
									<span className="text-[#F6BD7A] font-mono text-[10px] xs:text-xs lg:text-base">
										{character.title}
									</span>
									<span className="px-1.5 py-0.5 bg-[#F6BD7A]/20 rounded-full text-[#F6BD7A] text-[10px] lg:text-sm font-mono">
										{character.status}
									</span>
								</div>
											...
											<div className="flex items-center gap-2">
												...
												<div className="flex items-center gap-2">
													<a
														...
														className="text-[#F6BD7A] hover:text-[#F6BD7A] transition-colors flex items-center gap-1.5"
													>
														<span className="text-[10px] lg:text-sm">
															{shortenAddress(character.wallet)}
														</span>
								...
								<div className="flex flex-col xs:flex-row items-start xs:items-center gap-2 mt-3">
									<a
										...
										target="_blank"
										rel="noopener noreferrer"
										className="w-full xs:w-auto inline-flex items-center gap-1.5 px-2 py-1 lg:px-3 lg:py-1.5 bg-[#F6BD7A]/20 rounded-lg 
                             text-[#F6BD7A] hover:bg-[#F6BD7A]/30 transition-colors font-mono text-sm"
									>
                             ...
                             border border-[#F6BD7A]/30 hover:border-cyan-400/50 transition-colors"
									>
										<span className="text-[#F6BD7A] font-mono text-[10px] lg:text-sm">
											{getChatCommand()}
										</span>
										{copied ? (
											<Check className="w-3 h-3 lg:w-4 lg:h-4 text-[#F6BD7A]" />
										) : (
								...
								{/* Bio Section */}
								<div
									className="mt-4 font-mono text-[10px] xs:text-xs lg:text-sm text-gray-300 leading-tight lg:leading-relaxed 
                             whitespace-pre-line bg-black/40 p-2 lg:p-3 rounded-lg border border-[#F6BD7A]/30"
								>
									{character.bio}
								</div>
							</div>
						...
						{/* Right Column - Chat */}
						<div className="flex flex-col h-full border-t lg:border-t-0 lg:border-l border-[#F6BD7A]/30 overflow-hidden mt-0">
							{/* Chat Section */}
							<div className="flex flex-col flex-1 overflow-hidden">
								...
								<form
									...
									className="flex-shrink-0 p-2 lg:p-3 border-t border-[#F6BD7A]/30 bg-black/40"
								>
									<div className="flex items-center gap-2">
										<input
											ref={inputRef}
											...
											className="flex-1 bg-black/40 text-[#F6BD7A] placeholder-[#F6BD7A]/50 px-2 py-1.5 lg:px-3 lg:py-2 rounded-lg 
                               border border-[#F6BD7A]/30 outline-none font-mono text-xs sm:text-sm
                               focus:border-cyan-400/50 transition-colors"
										/>
										<button
											type="submit"
											disabled={!chatInput.trim()}
											className="p-2 rounded-lg bg-black/40 border border-[#F6BD7A]/30 
                               text-[#F6BD7A] hover:text-cyan-400 hover:border-cyan-400/50 
