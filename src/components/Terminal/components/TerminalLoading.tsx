...
export const TerminalLoading: React.FC<TerminalLoadingProps> = ({
	...
	return (
		<div className="flex flex-col gap-1 font-mono">
			...
			{subtext && (
				<p className="text-[#F6BD7A]/80 text-xs xs:text-sm whitespace-normal leading-relaxed">
					{subtext}
				</p>
			)}
			{rateLimit && (
				<div className="flex flex-col gap-1 mt-2 text-[10px] xs:text-xs">
					<div className="flex flex-col xs:flex-row xs:items-center gap-1 xs:gap-2">
						...
						<span className="text-[#F6BD7A]">
							{rateLimit.remaining}/{rateLimit.total} attempts remaining this
							window
						</span>
					</div>
					<p className="text-[#F6BD7A]/70 break-words">
